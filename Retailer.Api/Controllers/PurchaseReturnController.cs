...
namespace Retailer.Api.Controllers
{
    ...
    {
        private readonly IPurchaseReturnService _svc;
        ...
        public PurchaseReturnController(IPurchaseReturnService svc, RetailerDbContext context) { _svc = svc; _context = context;
		...
		[HttpPut("{id:int}")]
		public async Task<IActionResult> Update(int id, [FromBody] PurchaseReturnMasterDto model)
		{
			...
			try
			{
				...
				var existing = await _context.PurchaseReturnMasters
					.Include(p => p.Details)
				...
				foreach (var rem in toRemove)
					_context.PurchaseReturnDetails.Remove(rem);
				...
				foreach (var d in model.Details)
				{
					...
					{
						existing.Details.Add(new PurchaseReturnDetail
						{
							ItemId = d.ItemId,
							Rate = d.Rate,
							Qty = d.Qty,
        ...
        {
            var existing = await _context.PurchaseReturnMasters.Include(x=>x.Details).FirstOrDefaultAsync(b => b.Id == id);
            if (existing == null) return NotFound();
            existing.Active = 0;
            _context.PurchaseReturnMasters.Update(existing);
			foreach (var i in existing.Details)
			{
        ...
        public async Task<IActionResult> GetVendorWise(int vendorID, DateTime sdate, DateTime edate)
        {
            var list = await _svc.GetVendorWiseAsync(vendorID, sdate, edate, CompanyId);
            return Ok(list);
        }
        [HttpGet("ItemWise/{itemID}/{sdate}/{edate}")]
        public async Task<IActionResult> GetItemWise(int itemID, DateTime sdate, DateTime edate)
        {
