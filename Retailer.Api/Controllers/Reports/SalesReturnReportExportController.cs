...
namespace Retailer.Api.Controllers.Reports
{
    ...
    [ApiController]
    [Authorize]
    public class SalesReturnReportExportController : ControllerBase
    {
        private readonly ISalesReturnReportExportService _service;

        public SalesReturnReportExportController(ISalesReturnReportExportService service)
        {
            _service = service;
        }
        ...
        {
            var bytes = await _service.GenerateSalesReturnReportAsync(sdate, edate, CompanyId, export);
        ...
        {
            var bytes = await _service.GenerateCustomerSalesReturnReportAsync(customerId, sdate, edate, CompanyId, export);
            return File(bytes, ResolveContentType(export), BuildFileName("PurchaseReturnReport", export));
        }
        ...
        {
            var bytes = await _service.GenerateItemSalesReturnReportAsync(itemId, sdate, edate, CompanyId, export);
