...
{
    [Authorize]
    public class ItemWisePurchaseReturnModel : PageModel
    {
        private readonly IApiClient _api;

        public ItemWisePurchaseReturnModel(IApiClient api)
        {
            _api = api;
        }
        ...
        public async Task<IActionResult> OnGetLoadPurchaseReturn(int? itemCode, DateTime? sdate, DateTime? edate)
        {
            ...
            var data = await _api.GetPurchaseReturnItemWiseAsync(itemCode.Value, sdate.Value, edate.Value);
        ...
        public async Task<IActionResult> OnGetExportAsync(string export)
        {
            ...
            var bytes = await _api.ExportPurchaseReturnItemWiseAsync(ItemCode.Value, export, sdate.Value, edate.Value);
